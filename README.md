...
4. 哔哩哔哩 Basic
5. 酷狗、咪咕、酷我公开音源补充层
6. 网易云全量、QQ 音乐全量、B 站音视频范围内全量
7. 酷狗、咪咕、酷我其余全量能力（酷我接入前重新验证接口可用性）

Basic 优先形成可用播放器链路：必要协议、登录与多账户、直接搜索、个人歌单与 Uni 导入、歌词、播放/下载及权益，然后补目录与低频展示。酷狗、咪咕和酷我会先作为公开音源接入搜索、元数据、Uni Playlist 和跨平台回退，再补登录、账户及其余全量能力。具体范围见[路线图](docs/implementation-plan.md)，当前覆盖状态见 [`docs/coverage`](docs/coverage/)。
