...
Client 托管模式、服务端多歌单目录、版本化 `tuneweave_uni_playlist_v1` 交换文档和存储分片/嵌入式数据库改造尚未实现，已作为 B 站当前主线收口后的独立阶段规划。客户端模式将由调用方持有歌单数据，TuneWeave 只无状态地展开来源、标准化项目和播放当前项目；两种模式只通过显式导入/导出复制，不提供自动 `both` 同步。详细边界见 [Uni Playlist 客户端托管与存储设计](docs/uni-playlist-ownership.md)。
