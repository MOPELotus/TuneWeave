...
2. 接入酷狗、咪咕和酷我的公开音源补充层。
3. 补齐后台日志与测试版可观测性，执行全端点真实发布验收。
4. 实施 B 站延期的 GeeTest、密码与短信登录链。
5. 依次完成酷狗、咪咕和酷我的完整项目范围。
6. 六个平台稳定后实施验证码服务本地 Rust 版本。
7. 只在出现明确需求时，从平台功能扩展候选池选择能力实施。

“项目范围”是平台在 TuneWeave 产品定位内计划完成的全部正式能力，不是精简版或通往“全量覆盖”的过渡阶段。参考项目中超出检索、整理、播放和必要账户体验的接口仅进入扩展候选池，不参与完成率。酷狗、咪咕和酷我先以公开搜索、元数据、歌词、Uni Playlist 与播放回退增强音源覆盖；这一层完成后，再完成后台日志与全端点真实验收，即可发布首个 pre-release。具体范围见[路线图](docs/implementation-plan.md)，当前覆盖状态见 [`docs/coverage`](docs/coverage/)，发布门槛见[测试版验收标准](docs/pre-release-acceptance.md)。
