...
服务端多歌单目录已可通过 `GET /v1/uni/playlists` 分页读取，不再要求调用方自行记住全部 `uni:<id>`；`PATCH /v1/uni/playlists/{ref}` 可独立修改名称或描述，`DELETE` 可原子删除整份歌单及其项目。Client 托管使用强类型 `tuneweave_uni_playlist_v1` 交换文档，格式、身份、顺序、重复项、来源、快照及安全扩展边界已经固定；服务端歌单可通过 `/export` 取得不含凭据和临时传输信息的完整客户端副本。客户端文档原子导入、无状态处理接口和存储分片/嵌入式数据库改造仍在本阶段继续实施。客户端模式将由调用方持有歌单数据，TuneWeave 只无状态地展开来源、标准化项目和播放当前项目；两种模式只通过显式导入/导出复制，不提供自动 `both` 同步。详细边界见 [Uni Playlist 客户端托管与存储设计](docs/uni-playlist-ownership.md)。
