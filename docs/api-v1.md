...
| GET | `/v1/podcasts` | `platform?`、`account?`、`catalog`、`category_id?`（也接受 `categoryId`）、`limit?`、`offset?` | `Podcast[]`；统一目录类型由 `catalog` 选择，当前网易云支持 `featured`、`hot`、`category_featured`、`category_hot` 与 `personalized` |
