...
账户别名的作用域是平台，同名的 `netease/personal`、`qq/personal`、`bilibili/personal` 与 `soda/personal` 是四份独立登录态。兼容模式由服务器保存登录后的必要凭据，重启时按 `platform/account` 恢复；不存在的非默认别名不会回退到 `default`。QQ、网易云、B 站与汽水二维码登录支持 [`调用方托管凭证契约`](credential-ownership.md) 的 `credential_mode=server|client|both`：调用方可选择保持服务器托管、完全不落盘而接收凭证，或同时保存和返回同一代际；后续请求可通过专用请求头携带一至多平台凭证。密码、验证码和二维码事务本身始终不持久化。QQ、网易云与 B 站的调用方凭证刷新与退出已经接通；汽水当前先覆盖二维码创建、轮询及确认后的凭证归属，刷新、退出和账户业务端点按项目范围继续实施。
...
`POST /v1/account/podcasts/{ref}/episodes` 支持两种账户声音上传形态。兼容形态使用原始音频请求体，以请求 `Content-Type` 表示音频类型，并在查询中提供 `filename` 与已有 `cover_image_id`；multipart 形态使用 `audio`/`songFile`/`song_file` 音频文件字段，并可用 `cover`/`imgFile`/`cover_image` 同时提交封面文件，音频文件名也可由查询 `filename` 覆盖。封面文件与 `cover_image_id` 互斥且必须选择其一；音频最大 500 MiB，封面最大 20 MiB，文件名、图片类型和账户归属都在发网前校验。查询还必须给出 `category_id/second_category_id/description`，并可给出 `name/privacy/publish_time_ms/auto_publish/auto_publish_text/order_no/composed_songs/account`。所有字段兼容参考 camelCase 名称；布尔值接受 `true/false/1/0`，`order_no` 缺省 1 且最小 1，发布时间缺省 0（立即发布），`composed_songs` 接受逗号分隔的同平台裸歌曲 ID 或完整引用并保留顺序和重复项。音频和封面字节只存在于脱敏请求模型与上传事务中，不进入 JSON、Debug 或扩展。封面文件先复用受限图片上传链取得图片 ID，随后网易云实现完整执行 WeAPI `/api/nos/token/alloc`（`ymusic`）→ 固定 `ymusic.nos-hz.163yun.com` 的 10 MiB NOS 分片上传与 XML 完成 → EAPI `/api/voice/workbench/voice/batch/upload/preCheck` → EAPI `/api/voice/workbench/voice/batch/upload/v2`；两次提交按参考行为使用不同 RFC 4122 v4 `dupkey`，并携带 NOS token 请求头，但 token 不会写入结果或日志。空凭据真实服务器验证完整输入在发网前稳定返回 401；当前普通账户 `djStatus=0` 且没有可写播客目录，真实上传、发布后详情与播放验收留待创作者账户及可丢弃音频。
...
| POST | `/v1/account/podcasts/{ref}/episodes` | 原始音频 body + `cover_image_id`，或 multipart 的音频与可选封面文件；查询含分类、说明和可选发布、隐私、排序、包含歌曲及 `account` 参数 | `PodcastEpisodeUploadResult`；封面文件先上传取 ID，再完成音频令牌、NOS 分片、预检查与正式提交事务 |
...
| GET | `/v1/users/{ref}/favorites/playlist` | `account?` | 指定用户可见的“喜欢”歌单 `Playlist`；保留平台原封面 |
...
QQ、网易云、B 站与汽水当前均声明 `caller_managed_credentials`：所有会创建登录态的首个请求接受 `credential_mode=server|client|both`，多步二维码/验证码事务固定首请求选择，确认阶段不能改写。`client/both` 的成功登录或刷新响应新增一次性 `caller_credential`，并强制 `Cache-Control: no-store` 与 `Pragma: no-cache`；普通状态、账户和业务响应仍不回显凭证。业务请求以可重复 `X-TuneWeave-Credential` 请求头按平台携带，和同平台显式 `account/accounts` 冲突时返回 400。QQ 与网易云当前已实现的业务端点、跨平台回退及安全原始扩展已经接通；B 站已覆盖二维码登录、会话状态、账户资料、刷新和退出；汽水已接通二维码确认后的服务器/调用方凭证归属，业务消费、会话、刷新和退出仍随项目范围逐项接入。这只表示凭证所有权桥覆盖已实现端点，不代表对应平台的扩展候选已经实施。封装格式、大小限制、跨平台组合、刷新/退出与脱敏要求以 [`docs/credential-ownership.md`](credential-ownership.md) 为准。
...
| GET | `/v1/account/favorites/playlist` | `platform`、`account?` | 当前账户的“喜欢”歌单 `Playlist`；保留平台原封面 |
...
| GET | `/v1/account/favorites/tracks/intelligence` | `platform`、`account?`、`seed`、`start?`、`count?` | `FavoriteIntelligenceQueue`；以喜欢歌单为来源的心动模式/智能播放队列 |
...
网易云的当前账户和指定用户喜欢歌单端点固定读取 `/api/user/playlist`，只选择 `specialType=5` 且创建者与目标用户一致的歌单。返回名称使用平台资料昵称生成 `{username}喜欢的音乐`，昵称缺失或为空时为“我喜欢的音乐”；不会用平台原生的通用标题遮蔽昵称，但会在扩展中保留原名。封面、描述、曲目数、创建者和时间等其他歌单元数据保持平台原值。当前账户先从精确登录态取得用户 ID，指定用户端点使用路径引用并允许可选查看者账户；Uni `favorite_tracks` 导入复用同一份元数据，因此来源名称和 `cover_url` 与该歌单一致。
...
QQ 喜欢歌曲与喜欢歌单元数据使用同一 `CgiGetDiss` 的 `disstid=0/dirid=201` 分支。`GET /v1/account/favorites/playlist?platform=qq&account=...` 和 `/tracks` 从所选凭据读取 `encryptUin`；`GET /v1/users/qq:<encrypted-uin>/favorites/playlist` 和 `/tracks` 直接使用目标用户的加密 UIN，并允许可选 `account` 作为查看者会话。歌单元数据保留 QQ 原生“我喜欢”名称及对应原封面，Uni `favorite_tracks` 来源也复用该名称和 `cover_url`。该分支固定发送 `tag=true/userinfo=true/orderlist=true`，不发送只属于普通歌单精简取曲的 `onlysonglist`；歌曲端点共享严格 offset/limit、分页一致性、零进度拒绝和 Track 映射。加密 UIN 作为不透明平台用户 ID 处理，不接受空白、控制字符或超长值，也不会以参考项目的占位凭据代替缺失账户。普通账户的两条歌曲路径均以两页完整返回 158 首歌曲，身份、顺序、总数和终止状态完全一致。

网易云心动模式使用 `GET /v1/account/favorites/tracks/intelligence`，要求 `seed` 是所选平台的歌曲引用；`start` 省略时使用同一首歌，`count` 默认 1 且范围为 1–100。Provider 先定位当前账户的真实喜欢歌单，再调用 `/api/playmode/intelligence/list`，逐项保留平台 `recommended` 与 `alg`，并校验外层歌曲 ID 和完整歌曲对象身份一致。`count` 保留平台请求语义，不作为统一 offset 分页大小；返回的是当前队列快照，不附加伪造的分页或 continuation。
...
汽水音乐支持 `login_type=default/web/soda`，固定访问官方 PC Passport 的二维码创建与轮询端点。登录设备身份只在首次认证事务时惰性生成并持久化，公开搜索和播放不会创建该状态；同一事务复用设备身份和随机 trace，当前真实协议不需要 `msToken` 或 `a_bogus`。平台返回的二维码必须是 1 MiB 内的有效 PNG；缺图兼容分支只会把严格受信的 `https://bff-pc.qishui.com/ucenter_web/app/sdk-next` URL 在进程内编码为 SVG，不会访问响应指定的任意地址。上游 token、Cookie 和设备参数封装在最多 128 个、有效期 5 分钟的随机外层事务中，轮询最短间隔为 2 秒，平台限流触发 60 秒冷却；响应区分未扫码、已扫码、需要附加验证、过期、失败和确认。确认 Cookie 经过数量、名称、值和总大小校验后，才按首请求固定的 `credential_mode` 原子保存或一次性返回。创建和未扫码轮询已真实验证；普通账户扫码确认、附加短信验证、会话刷新与退出仍按汽水项目范围继续验收和实施。
...
B 站 `video` 类型 Uni 项默认把视频视为可播放音频来源：未提交 `resolution` 时先用项目原始 B 站身份选择 BM02 音轨，`quality` 与 `accounts.bilibili` 指定的服务器账户或调用方托管凭证会进入同一播放请求。成功响应的统一 `MediaStream` 保留实际音质、码率、格式、编码、备用 URL、到期时间、必要 `Referer` 和固定 Web `User-Agent`，`extensions.transport=native_video_audio` 与完整强类型音轨结果用于区分来源；不会返回账户 Cookie。真实 CDN 差分确认只带 Referer 会返回 403，同时带固定 User-Agent 后受限 Range 请求返回 206 和媒体字节。显式提交 `resolution=1..4320` 时改用原生视频轨，并标记 `transport=native_video`。Uni 项的 `/stream/redirect` 同样只是无缓存 302，无法代传 `MediaStream.headers`；需要这些请求头的客户端必须使用 JSON 端点自行发起媒体请求。

只有原始 B 站音轨失败且 `fallback=true` 时，解析器才按 `playback_platform/fallback_platforms` 顺序搜索其他音乐平台；匹配使用 Uni 快照中的标题、艺人、时长、ISRC 和版本标签，保持严格阈值，翻唱、现场或版本冲突不会因“能播放”而被接受。每个平台可用 `accounts` 提交独立账户，全部候选、分数、账户和失败分类按尝试顺序返回。默认音乐回退列表不包含 B 站，普通歌曲不会反向搜索 B 站视频；B 站视频是原始项目时仍自动先尝试自身音轨。公开视频 `BV1Jt411P77c` 已通过真实 Uni HTTP 取得原生音轨，账户隔离、原音轨失败后 QQ 严格命中及显式视频轨分支完成自动化验收。普通非大会员账户请求 `master` 时真实降级为普通 `higher` 音轨，保留 URL、Referer 和固定 Web User-Agent；同一视频经 Client Uni 无状态播放仍选择 B 站且不持久化，未伪造大会员权益。
