...
| GET | `/v1/videos` | `platform?`、`account?`、`catalog=all|latest|exclusive`、`area?`、`type?`、`order?`、分页 | `Video[]`；独立 MV 目录，按后端真实能力约束筛选及续页 |
...
| GET | `/v1/recommendations/podcast-episodes` | `platform?`、`account?`、`limit?`、`offset?` | `PodcastEpisode[]` 推荐节目固定快照 |
...
网易云 MV 收藏写入精确覆盖 `mv_sub`：统一 PUT/DELETE 分别调用 WeAPI `/api/mv/sub` 与 `/api/mv/unsub`，同时提交数值 `mvId` 和参考格式的字符串 `mvIds=["..."]`。该模块不支持普通站内视频收藏写入，因此 `kind=video` 会在发网前明确拒绝；数值引用省略类型时仍按统一视频规则推断为 MV。`mv_sublist` 固定调用 WeAPI `/api/cloudvideo/allvideo/sublist`，提交 `limit/offset/total=true`，将上游混合返回的字符串 `vid`、创作者、封面、时长、播放量和来源 `type` 映射为已收藏 `Video[]`，完整单项及去除大数组后的分页响应分别保存在扩展。2026-07-22 持久化真实账户已验证列表读取，并对公开 MV 完成未收藏→收藏→取消收藏的写入回滚，最终状态与测试前一致。

推荐节目固定使用 WeAPI `/api/personalized/djprogram`，外层推荐包装中的 `program` 映射为完整 `PodcastEpisode`；节目、所属播客和承载音频三种引用保持分离，并可直接复用节目取流链路。该接口不接受分页控制，因此统一端点只允许 `offset=0`，`limit` 是本地快照上限，分页扩展明确 `continuation_supported=false/limit_applied=false`。2026-07-18 匿名真实联网已一次覆盖上述推荐新歌、歌单、MV、节目、独家放送入口及独家放送列表六个分支，均返回非空类型化资源。
