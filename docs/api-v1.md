...
| GET | `/v1/podcasts` | `platform?`、`account?`、`catalog`、`category_id?`（也接受 `categoryId`）、`limit?`、`offset?` | `Podcast[]`；统一目录类型由 `catalog` 选择，当前网易云支持 `hot` |
...
播客目录的 `catalog` 必填并采用跨平台稳定名称；平台尚未实现的目录会明确返回 `invalid_request`，不会静默换成另一种目录。网易云当前支持 `catalog=hot`，固定使用 WeAPI `/api/djradio/hot/v1` 并精确提交 `limit/offset`；`limit` 默认为 30、范围 1–100，`offset` 默认为 0，该目录不接受 `category_id`。统一结果映射封面、主播、分类、节目数、订阅数、播放数、付费态与创建时间；上游未提供可靠总数时 `total=null`，真实 `hasMore` 决定 `has_more/next_offset`，每项原文及完整响应都保存在扩展中。
