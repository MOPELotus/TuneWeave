...
| `X-TuneWeave-Credential` | 计划中的可重复敏感请求头；每项携带一个平台的调用方托管凭证，不进入 URL、普通 JSON、日志或扩展字段。 |
...
账户别名的作用域是平台，同名的 `netease/personal` 与 `qq/personal` 是两份独立登录态。现有兼容模式由服务器保存登录后的必要凭据，重启时按 `platform/account` 恢复；不存在的非默认别名不会回退到 `default`。QQ Basic 基础单元收口后将按 [`调用方托管凭证契约`](credential-ownership.md) 增加 `credential_mode=server|client|both`：调用方可选择保持服务器托管、完全不落盘而接收凭证，或同时保存和返回；后续请求可通过专用请求头携带一至多平台凭证。密码、验证码和二维码事务本身始终不持久化。
...
QQ 的 `client=mobile` 精确对应 Android `music.smartboxCgi.SmartBoxCgi/GetSmartBoxResult`：普通 `items` 与 `vec_related_items` 分别进入 `suggestions/recommendations`，`vec_direct_items` 依据 `insert_pos` 插回建议序列并尽可能提升为统一资源。搜索会话 ID 位于列表扩展；高亮展示、图标、跳转、分值、预搜索标志、关联资源 ID 及完整上游包装逐项保留。`client=web` 精确对应参考 `quick_search` 的固定 HTTPS `c.y.qq.com/splcloud/fcgi-bin/smartbox_new.fcg`：各分区按上游 `order` 动态排序，单曲、歌手、专辑和 MV 提升为统一资源，未来未知分区以保留完整原项的 `opaque` 表达；分区元数据和完整响应保存在扩展。`pc` 没有独立 QQ 上游分支，会明确报错而不会偷换成 web/mobile 结果。2026-07-22 release 统一 HTTP 真实返回 21 项移动端建议（首项为歌手直达资源）和 10 项 web 快速结果（首项为“晴天”歌曲，上游 `code/subcode=0`）。
...
调用方托管凭证扩展是 QQ Basic 平台基础单元完成后的第一项跨平台工作，当前服务尚未声明 `caller_managed_credentials` 能力时仍只接受服务器账户别名。接入后，所有会创建登录态的首个请求接受 `credential_mode=server|client|both`；多步二维码/验证码事务固定首请求选择，确认阶段不能改写。`client/both` 的成功响应新增一次性 `caller_credential`，并强制 `Cache-Control: no-store`；普通状态、账户和业务响应仍不返回凭证。业务请求以可重复 `X-TuneWeave-Credential` 请求头按平台携带，和同平台显式 `account/accounts` 冲突时返回 400。封装格式、大小限制、跨平台组合、刷新/退出与脱敏要求以 [`docs/credential-ownership.md`](credential-ownership.md) 为准。
...
QQ 专辑收藏写入通过普通 Android `music.musicasset.AlbumFavWrite` 完成：收藏调用 `FavAlbum`，取消收藏调用 `CancelFavAlbum`。单项 `PUT|DELETE /v1/account/library/albums/qq:<numeric-id>` 与批量 `PUT|DELETE /v1/account/library/albums` 使用同一批量实现；批量体可提交完整 `refs`，也可提交 `platform=qq` 与 `ids`，保留原始顺序和重复项并一次发送 `v_albumId`。专辑 ID 必须是正整数，输入在账户读取和网络访问前完成校验；凭据严格取自所选 `(qq, account)`。只有包络码与强类型 `result` 同时为零且 `v_failedAlbumId` 为空才返回成功，部分失败、未知失败 ID、缺字段或畸形响应不会被误报为已改变。Q051/Q052 两个方向完成后才宣告 `album_subscription_write` 能力。
