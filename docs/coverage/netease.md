...
当前统计：`pending=251`、`partial=3`、`implemented=57`、`verified=96`。只有所有条目都达到 `verified`，或以证据明确标为上游已失效，网易云阶段才算完成。
