...
| 服务端多歌单目录与元数据管理 | `implemented` | 分页 `GET /v1/uni/playlists` 已接入内存与文件存储，按不可变创建时间降序、同时间 ID 升序稳定列出服务端歌单，只返回元数据和项目数而不内联项目；默认 `limit=50/offset=0`，范围、总数、下一偏移、空页、未知参数及文件重启读取均已验证。明确的 `PATCH/DELETE /v1/uni/playlists/{ref}` 仍待实现，完成前本单元不升级为 `verified`。 |
