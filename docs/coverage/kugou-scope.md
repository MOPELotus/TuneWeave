...
| KG07 | 移动协议与匿名设备身份 | `verified` | 启动时在私有 `kugou-device.json` 原子持久化匿名 GUID 及其确定性派生 MID，首次真正进入移动协议时才通过固定 HTTPS `/risk/v2/r_register_dev` 注册 `dfid`；AES-CBC 请求档案、RSA PKCS#1 v1.5 注册密钥、Android 签名和加密响应均在 Rust 内部完成，调用方不能注入身份、URL、请求头或签名。并发首次请求由单一事务收口。真实验证覆盖首次注册、重启复用、已注册旧 GUID 不补发 `dfid` 的成功空数据分支、1 秒冷却后轮换完整身份且仅重试一次，以及刷新后的歌曲详情、权益和完整公开播放；畸形本地 `dfid` 会失效，GUID/MID 不一致则拒绝启动，不会无限刷新或静默使用损坏状态。 |
