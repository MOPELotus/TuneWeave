...
当前公开音源补充层共 7 个验收单元：`pending=5`、`implemented=0`、`verified=2`，完成度 `2/7 = 28.57%`。登录、账户和写操作属于后续酷狗完整项目范围，不阻塞本层。
...
| KG03 | 普通与逐字歌词 | `pending` | 接入歌词候选搜索及 `id + accesskey` 下载，区分 LRC/KRC、翻译和逐字时间轴，不允许普通歌词覆盖更高精度结果。 |
