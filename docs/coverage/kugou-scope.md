...
当前公开音源补充层共 7 个验收单元：`pending=6`、`implemented=0`、`verified=1`，完成度 `1/7 = 14.29%`。登录、账户和写操作属于后续酷狗完整项目范围，不阻塞本层。
...
| KG02 | 歌曲详情与身份补全 | `pending` | 以 `album_audio_id + hash` 读取详情，补全版本、ISRC、发行信息和全部可用媒体规格；输入身份不能混用。 |
