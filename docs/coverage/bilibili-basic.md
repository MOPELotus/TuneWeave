...
状态沿用其他平台账本：`pending` 尚未实现，`partial` 缺少必要分支，`implemented` 已完成代码和离线验证但缺真实账户或真实网络成功态，`verified` 已完成对应真实路径验收。当前共 34 个验收单元：`pending=20`、`partial=0`、`implemented=6`、`verified=8`；完整实现率与已触达率均为 `14/34 = 41.18%`。
