...
最后更新：2026-07-18。上游基线与逐模块状态仍以 [`netease.md`](netease.md) 的 407 项全量账本为准；本表只把 Basic 范围聚合成可独立验收的能力单元，不替代或合并掉任何上游 API。
...
| C11 | 内容展示 | 声音及声音歌单详情、目录和歌词 | `partial` | `voice_lyric` 已通过 provider 与真实统一 HTTP 验收，覆盖 675 段非空转写和 `data=null`；`voice_detail` 与 `voicelist_detail` 分别以 `/v1/episodes/{ref}?backend=workbench`、`/v1/podcasts/{ref}?backend=workbench` 接入独立能力和类型化详情，兼容工作台字段、完整对象优先于低信息摘要并实测匿名 301 认证边界，成功态留待 Basic 末尾集中验收；声音歌单目录/搜索仍待接入 |
...
| P10 | 播放与权益 | 播客、电台节目和声音播放地址 | `partial` | 节目先解析独立 `audio.ref`，再复用完整歌曲音质、VIP、账户、跨平台回退和 302 链路；声音逐词转写已接入且真实验证，工作台详情会把 `songId/trackId` 稳定映射为独立 `audio.ref`，待登录成功态确认后即可复用现有取流；声音歌单目录/播放仍待接入 |
