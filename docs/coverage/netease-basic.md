...
| A09 | 账户与身份 | 会话刷新及退出 | `implemented` | 2026-07-17 服务器托管真实账户刷新、凭据代际替换和重启恢复均已验收；现已补齐调用方与 `both` 模式，刷新只在新 Cookie 及会话状态均有效后保存/返回，`both` 按 `MUSIC_U` 在发网前核对身份，退出分别保持客户端存储边界并删除精确服务器别名。调用方成功态与退出仍待需要重新扫码时受控验证 |
