...
| C11 | 内容展示 | 声音及声音歌单详情、目录和歌词 | `partial` | `voice_lyric` 已通过 provider 与真实统一 HTTP 验收，覆盖 675 段非空转写和 `data=null`；`voice_detail`、`voicelist_detail` 与 `voicelist_list` 分别以详情或节目目录的 `backend=workbench` 接入独立能力和类型化输出，兼容工作台字段、包装字段合并、无效首选列表回退及最大 200 条分页，并实测匿名 301 认证边界，成功态留待 Basic 末尾集中验收；工作台声音搜索仍待接入 |
...
| P10 | 播放与权益 | 播客、电台节目和声音播放地址 | `partial` | 节目先解析独立 `audio.ref`，再复用完整歌曲音质、VIP、账户、跨平台回退和 302 链路；声音逐词转写已接入且真实验证，工作台详情及声音歌单目录会把 `songId/trackId` 稳定映射为独立 `audio.ref`，待登录成功态确认后即可复用现有取流；工作台声音搜索仍待接入 |
