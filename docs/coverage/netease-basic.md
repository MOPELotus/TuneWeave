...
| F08 | 平台基础协议 | 设备身份、匿名 token 与实时 checkToken | `partial` | 请求层设备默认值已有；`register_checktoken_v3` 已完成双路径 GET/POST、缓存/刷新、日志脱敏、XEAPI 受控头注入及真实注册/强刷验收；上游新增的 v2 checkToken 及 `register_anonimous` 仍待接入 |
