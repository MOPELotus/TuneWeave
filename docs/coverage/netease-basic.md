...
| C07 | 内容展示 | MV/视频搜索、歌手视频目录和收藏态 | `partial` | 搜索、歌手目录、全部/最新/网易出品三个独立 MV 目录，以及 `mv_sub/mv_sublist` 收藏写入和账户混合视频收藏列表均已完成并真实验证；写入同时保留参考 `mvId/mvIds`，普通视频不会误走仅支持 MV 的收藏模块，真实闭环后账户状态已恢复；实际视频 ID、专用标题/封面、正时长及非空完整创作者优先于空/零包装摘要，最新目录不伪造 offset；站内视频独立目录仍缺 |
