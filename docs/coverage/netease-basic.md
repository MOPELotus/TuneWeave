...
当前剩余功能排序以完整播放体验为准：L11/L12 云盘写入、读取、详情、源文件下载、直接播放和删除已经用 TuneWeave 自建音频完成真实事务及完整回滚；匹配和文件内嵌歌词在 TuneWeave 与参考实现中均返回相同业务失败，不伪造成功态。主线已进入 C10/C11/P10 播客、电台节目与声音内容链路：分类、热门目录、详情、节目目录、节目播放/302 及声音逐词转写已经验收，精选、个性化、分类热门、分类精选、今日优选和付费目录代码已完成并留待 Basic 末尾集中联网验收，下一步补其他常规目录和声音歌单。
...
| C10 | 内容展示 | 播客/电台节目分类、详情和节目列表 | `partial` | `dj_catelist/dj_hot/dj_detail/dj_program/dj_program_detail` 已通过 provider 与真实统一 HTTP 验收；`dj_recommend/dj_personalize_recommend/dj_radio_hot/dj_recommend_type/dj_today_perfered/dj_paygift` 已完成精选、个性化、分类、今日优选及付费目录映射和离线统一 HTTP 验证，独立页码与 offset 分离，付费价格转换为统一 CNY `Money`，留待 Basic 末尾集中联网验收；其他常规目录仍待接入 |
