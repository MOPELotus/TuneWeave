...
| C11 | 内容展示 | 声音及声音歌单详情、目录和歌词 | `partial` | `voice_lyric` 已通过 provider 与真实统一 HTTP 验收，覆盖 675 段非空转写和 `data=null`；`voice_detail` 已以 `/v1/episodes/{ref}?backend=workbench` 接入独立能力和类型化详情，兼容工作台字段并实测匿名 301 认证边界，成功态留待 Basic 末尾集中验收；`voicelist_*` 详情与目录仍待接入 |
