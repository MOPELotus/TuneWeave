...
| C11 | 内容展示 | 声音及声音歌单详情、目录和歌词 | `partial` | `voice_lyric` 已通过 provider 与真实统一 HTTP 验收，覆盖 675 段非空转写和 `data=null`；`voice_detail`、`voicelist_detail`、`voicelist_list` 与 `voicelist_list_search` 分别以详情/目录的 `backend=workbench` 或 `/v1/account/podcast-episodes` 接入独立能力和类型化输出，完整保留名称、七种审核状态、公开性、付费性、所属播客、包装字段合并、无效首选列表回退及最大 200 条分页，并实测匿名 301 认证边界，成功态留待 Basic 末尾集中验收；账户创建的声音歌单及写入链路仍待接入 |
