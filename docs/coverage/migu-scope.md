...
当前公开音源补充层共 7 个验收单元：`pending=6`、`implemented=0`、`verified=1`，完成度 `1/7 = 14.29%`。登录、账户和写操作属于后续咪咕完整项目范围，不阻塞本层。
...
| MG02 | 歌曲详情与身份补全 | `pending` | 以 `contentId` 查询当前资源详情，复核 `copyrightId/resourceType`，补齐发行、版本、ISRC、完整媒体规格和公开权益字段；不能用搜索快照冒充当前详情。 |
