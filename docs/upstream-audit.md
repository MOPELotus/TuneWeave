...
- 公开层先验证分享页能够匿名取得的稳定 ID、详情、歌手、专辑、封面、歌词和试听信息，再验证 PC 搜索能否用可持久化匿名设备运行。需要个人登录态、维护者 VIP、共享 Cookie 池或远程解析站的能力不能声明为公开能力。
- `x-helios`、`x-medusa`、设备身份和请求签名必须明确生成、有效期、轮换和重启恢复策略。搜索目录、媒体规格和 VIP 标志不能代替实时权益；匿名播放只返回实际获得的完整媒体或准确试听窗口。
