...
- `SearchApi.search_by_type` 新增 `RINGTONE=10`、请求 `selectors + vec_selectors` 和二维 selectors 响应。真实差分确认彩铃使用 `item_song` 且“周杰伦”目录总数为 553；selector 请求成功但参考实现对重复类型存在映射末项覆盖、向量全部保留的歧义。TuneWeave 已增加独立 `ringtone` 类型和强类型 selector，同类型重复项联网前拒绝，Python 上游、Rust provider 与统一 HTTP 均完成新增分支真实验收；十类逐项 Rust 回归同时发现并补齐当前用户结果的 `title/subtitle/iconurl` 字段优先级。
- `SongApi.query_song` 改为逐项 `id|mid + song_type`。真实差分确认两个数字 ID 的 `types=[1,113]` 可返回普通/特殊歌曲，而参考实现单子请求混合 `ids+mids` 的两个顺序都返回 `103901`。TuneWeave 已改为强类型逐项输入，在同一 HTTP 批包中拆成两个合法 CGI 子请求、分别对齐类型并恢复原始顺序/重复项，Rust provider 和统一 HTTP 均真实通过。
- `LyricApi.get_lyric` 新增助唱标注分支，同时新增助唱存在性、多风格翻译、AI 词典存在性和详情 4 个公开方法；QQ Basic 分母由 73 增至 77，全量分母由 100 增至 104。助唱内容/时间戳和逐请求布尔保留已完成，数字 ID `97773` 的上游 Python、Rust provider 与统一 HTTP 真实链均通过；其余 4 个独立方法继续按账本实施。
