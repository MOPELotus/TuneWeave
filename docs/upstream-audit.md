...
| 2026-07-26 | 完成 QQ 歌单创建、删除及歌曲写入后检查 | `QQMusicApi` | `873255f→873255f` | 无更新，104 个公开方法及创建/删除/歌曲增删协议均未变化 |
| 2026-07-26 | 完成 QQ 助唱、多风格翻译及 AI 词典存在性后检查 | `QQMusicApi` | `873255f→873255f` | 无更新，104 个公开方法及 Q101–Q103 歌词协议均未变化 |
| 2026-07-26 | 完成 QQ 歌手专辑、歌手 MV、批量详情及分类目录后检查 | `QQMusicApi` | `873255f→873255f` | 远端 `main` 无更新，104 个公开方法及 Q035/Q036/Q038/Q039 协议均未变化 |
| 2026-07-26 | 完成 QQ 歌手主页标签、批量详情及相似推荐后检查 | `QQMusicApi` | `873255f→873255f` | 远端 `main` 无更新，104 个公开方法及 Q031–Q033 协议均未变化 |
| 2026-07-26 | 完成 QQ 榜单目录、榜单歌曲及新专辑目录后检查 | `QQMusicApi` | `873255f→873255f` | 远端 `main` 无更新，104 个公开方法及 Q011/Q012/Q027 协议均未变化 |
| 2026-07-26 | 完成 QQ 歌手分类、索引及收藏专辑后检查 | `QQMusicApi` | `873255f→873255f` | 远端 `main` 无更新，104 个公开方法及 Q028/Q029/Q067 协议均未变化 |
| 2026-07-26 | 完成 QQ 收藏 MV、用户主页及关注歌手后检查 | `QQMusicApi` | `873255f→873255f` | 远端 `main` 无更新，104 个公开方法及 Q068/Q059/Q061 协议均未变化 |
| 2026-07-26 | 完成 QQ 不喜欢目录、添加及取消后检查 | `QQMusicApi` | `873255f→873255f` | 远端 `main` 无更新，104 个公开方法及 Q070–Q072 的签名读取、普通 CGI 添加/取消协议均未变化 |
| 2026-07-26 | 完成 QQ 清空不喜欢及专辑收藏/取消后检查 | `QQMusicApi` | `873255f→873255f` | 远端 `main` 无更新，104 个公开方法及 Q073 两阶段清空、Q051/Q052 批量专辑写入协议均未变化 |
| 2026-07-26 | 完成 QQ 综合搜索、首页推荐及推荐歌单后检查 | `QQMusicApi` | `873255f→873255f` | 远端 `main` 无更新，104 个公开方法及 Q005–Q007 的搜索分桶、推荐游标和歌单分页协议均未变化 |
| 2026-07-26 | 完成 QQ 分区新歌、猜你喜欢及雷达推荐后检查 | `QQMusicApi` | `873255f→873255f` | 远端 `main` 无更新，104 个公开方法及 Q008–Q010 的六区快照、五首推荐和雷达页码协议均未变化 |
| 2026-07-26 | 完成 QQ 相似歌曲、歌曲标签及相关歌单后检查 | `QQMusicApi` | `873255f→873255f` | 远端 `main` 无更新，104 个公开方法及 Q015–Q017 的请求签名未变化；真实差分确认参考响应模型漏掉 `vecSong/vecPlaylist`，且相关歌单误从稳定 `vecPlaylistNew` 派生换批游标，TuneWeave 已保留两支并改用真实直接分支游标 |
| 2026-07-28 | 修复并真实复验 QQ 二维码登录 | `QQMusicApi` | `873255f→873255f` | 上游 104 个公开方法无变化；真实 QQ 登录表明当前平台会沿 `check_sig` 跳转链分批下发 Cookie，并可能用同名域清理 Cookie 跟随有效 `p_skey`。TuneWeave 保留参考参数与状态分支，改为只在固定 QQ Graph HTTPS 主机间有界跟随并逐跳汇总非空 Cookie；QQ、微信、QQ 音乐客户端三类扫码确认均通过，QQ 凭据重启恢复及微信临时账户退出删除通过 |
...
| 2026-07-28 | 完成调用方凭证视频取流、歌曲关联资源、电台及播客详情后检查 | 全部 8 个参考仓库 | 网易云 `63d89aa`；Lotus `004bbff`；QQ `873255f`；酷狗 `283f1e9`；咪咕 `7759dae`；酷我 `e8e720b`；BBDown `259a555`；B 站文档 `cfc5fdd` | 全部当前快照无更新；QQ 旧审计副本继续保留历史分叉，`qqmusic-current` 精确对齐远端 `main@873255f` |
