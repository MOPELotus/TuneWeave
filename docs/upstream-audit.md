...
- `2026-07-29` 已重新拉取参考仓库，仍为 `e8e720b`、与远端无分歧；上游最后提交时间为 2023 年。
- 参考项目的旧 `/api/www/search/searchMusicBykeyWord` 当前返回 403 或非法请求，当前官网改用匿名 HTTPS `/search/searchMusicBykeyWord`；该链已真实返回稳定歌曲、100 项物理页和总数，TuneWeave 不回退过时入口。
...
- 移动端 `songinfoandlrc` 当前仍能返回行级歌词；付费样本的公开播放接口明确拒绝匿名全曲。后续逐项区分免费、会员和试听权益，只有真实可用的端点才声明能力。
