...
- 匿名 `track_v2.track_player.video_model` 当前以 `status=10` 返回 CENC AES-CTR 媒体；免费样本的 player ID 等于歌曲 VID 且时长覆盖全曲，会员样本的 player ID 等于 `preview/audition_info` VID 且只覆盖精确试听窗口。实现以身份和时长交叉回配实时权益，兼容 `backup_url` 的字符串/数组两种官方形态，只返回脱敏规格及解密要求；目录 VIP 标志、静态 player URL 和参考项目的共享凭据均不能替代该判断。
...
- 公开歌单已按平台稀疏原始 cursor 完整遍历，公开专辑只从固定官方分享页的有界路由 JSON 读取完整快照；两类来源均可用 `platform + type + id` 进入 Server 或 Client Uni Playlist，保持顺序和重复项。全新部署已真实合并 398 首歌单和 38 首专辑为 436 项，并验证内容交付、同源 302、试听下载拒绝、传输注入拒绝和敏感字段脱敏。登录、个人歌单、收藏、会员状态和登录权益播放仍留在后续完整项目范围。
