...
use crate::{
    AccountDislikeList, AccountDislikeListRequest, AccountDislikeMutationRequest,
    AccountDislikeMutationResult, AccountProfile, AiLyricDictionary, AiLyricDictionaryAvailability,
    Album, AlbumListRequest, AlbumStats, AnonymousSession, AntiCheatToken, AntiCheatTokenVersion,
    Artist, ArtistCatalog, ArtistCatalogRequest, ArtistChart, ArtistChartRequest,
    ArtistDescriptionRequest, ArtistHomepageTab, ArtistHomepageTabRequest, ArtistListRequest,
    ArtistOverview, ArtistStats, ArtistTrackListRequest, ArtistUpdatesRequest,
    ArtistVideoListRequest, ArtistWorkUpdate, ArtistWorksRequest, AudioCdnDispatch, AudioContent,
    AudioFileBatch, AudioFileRequest, AudioRecognition, AudioRecognitionRequest,
    AuthChallengeDelivery, AuthChallengeRequest, AuthChallengeValidation, AuthPrincipalStatus,
    AuthPrincipalStatusRequest, AuthSecurityChallengeRequest, Banner, BannerListRequest,
    Capability, ChartCatalog, ChartCatalogRequest, ChartTrackListRequest, CloudImportRequest,
    ...
    CommentReactionMutationResult, CommentReactionPage, CommentReportRequest, CommentReportResult,
    ...
    VideoSubtitleRequest, VideoTaxonomyRequest, VideoTrackStream, VideoTrackStreamRequest,
};
...
pub trait MusicProvider: Send + Sync {
    ...
    async fn general_search(&self, _request: &GeneralSearchRequest) -> Result<GeneralSearchResult> {
        Err(TuneWeaveError::unsupported(
            self.platform(),
    ...
    async fn anti_cheat_token(
        &self,
        _version: AntiCheatTokenVersion,
        _refresh: bool,
    ...
    async fn anonymous_session(&self, _refresh: bool) -> Result<AnonymousSession> {
        Err(TuneWeaveError::unsupported(
            self.platform(),
    ...
    async fn listening_rights_status(
        &self,
        _request: &ListeningRightsStatusRequest,
    ...
    async fn radio_playback_queue(
        &self,
        _id: &str,
        _request: &RadioPlaybackQueueRequest,
    ...
    async fn podcast_creator_chart(
        &self,
        _request: &PodcastCreatorChartRequest,
    ...
    async fn podcast_workbench(&self, _id: &str, _account: Option<&str>) -> Result<Podcast> {
        Err(TuneWeaveError::unsupported(
            self.platform(),
    ...
    async fn reorder_podcast_episode(
        &self,
        _podcast_id: &str,
        _request: &PodcastEpisodeOrderRequest,
    ...
    async fn delete_podcast_episodes(
        &self,
        _request: &PodcastEpisodeDeleteRequest,
    ...
    async fn upload_podcast_episode(
        &self,
        _podcast_id: &str,
        _request: &PodcastEpisodeUploadRequest,
    ...
    async fn artist_descriptions(
        ...
        for id in ids {
            artists.push(self.artist(id, request.account.as_deref()).await?);
        }
        Ok(artists)
    }
    ...
    async fn related_playlists(
        &self,
        _id: &str,
        _request: &RelatedPlaylistRequest,
    ...
    async fn track_credits(&self, _id: &str, _account: Option<&str>) -> Result<TrackCredits> {
        Err(TuneWeaveError::unsupported(
            self.platform(),
    ...
    async fn artist_catalog(&self, _request: &ArtistCatalogRequest) -> Result<ArtistCatalog> {
        Err(TuneWeaveError::unsupported(
            self.platform(),
    ...
    async fn video_track_stream(
        &self,
        _id: &str,
        _request: &VideoTrackStreamRequest,
    ...
    async fn account_dislikes(
        &self,
        _request: &AccountDislikeListRequest,
    ...
    async fn add_account_dislikes(
        &self,
        _request: &AccountDislikeMutationRequest,
    ...
    async fn audio_files(&self, _request: &AudioFileRequest) -> Result<AudioFileBatch> {
        Err(TuneWeaveError::unsupported(
            self.platform(),
    ...
    async fn send_auth_security_challenge(
        &self,
        _request: &AuthSecurityChallengeRequest,
