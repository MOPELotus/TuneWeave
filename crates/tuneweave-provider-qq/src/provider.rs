...
#[derive(Clone, Debug, Deserialize, Serialize)]
struct QqDislikeItem {
    #[serde(rename = "ID", deserialize_with = "deserialize_qq_string")]
    id: String,
    #[serde(rename = "Name")]
    name: String,
    #[serde(default, rename = "Img")]
    image_url: String,
    #[serde(rename = "IdType", deserialize_with = "deserialize_qq_i64")]
    id_type: i64,
    #[serde(rename = "Time", deserialize_with = "deserialize_qq_u64")]
    timestamp: u64,
    #[serde(flatten)]
    extra: BTreeMap<String, Value>,
}
...
#[derive(Clone, Debug, Deserialize, Serialize)]
struct QqAlbumSubscriptionResponse {
    #[serde(deserialize_with = "deserialize_qq_i64")]
    result: i64,
    #[serde(default, rename = "v_failedAlbumId")]
    failed_album_ids: Vec<u64>,
    #[serde(flatten)]
    extra: BTreeMap<String, Value>,
}
...
struct QqAiDictItem {
    ...
    lyric_text: String,
    #[serde(default)]
    trans_lyric_text: String,
    #[serde(default)]
    lyric_timestamp: String,
    #[serde(flatten)]
    extra: BTreeMap<String, Value>,
}

#[derive(Clone, Debug, Deserialize, Serialize)]
struct QqAiDictResponse {
    #[serde(default, rename = "dictList")]
    entries: Vec<QqAiDictItem>,
    #[serde(flatten)]
    extra: BTreeMap<String, Value>,
}
...
#[derive(Clone, Debug, Deserialize, Serialize)]
struct QqAlbumSongResponse {
    #[serde(rename = "albumMid")]
    album_mid: String,
    #[serde(rename = "totalNum", deserialize_with = "deserialize_qq_u64")]
    total: u64,
    #[serde(default, rename = "songList")]
...
impl MusicProvider for QqProvider {
    ...
    async fn artist_videos(
        ...
        if !(1..=100).contains(&request.limit) {
            return Err(TuneWeaveError::invalid_request(
                "QQ singer MV page size must be between 1 and 100",
            )
            .with_platform(Platform::Qq));
        }
        if request.cursor.is_some() {
            return Err(TuneWeaveError::invalid_request(
                "QQ singer MV list does not support cursors",
            )
            .with_platform(Platform::Qq));
        }
        if request
            .order
            .as_deref()
            .is_some_and(|order| !order.eq_ignore_ascii_case("hot"))
        {
            return Err(TuneWeaveError::invalid_request(
                "QQ singer MV list only exposes the upstream hot order",
            )
            .with_platform(Platform::Qq)
            .with_details(json!({ "allowed_order": "hot" })));
        }
        let request_api = qq_singer_mvs_request(id, request.limit, request.offset)?;
        let response = self
            .client
            .request_android(&[request_api])
            .await?
            .into_iter()
            .next()
            .ok_or_else(|| qq_data_error("QQ singer MV request returned no response"))?;
        map_qq_singer_mvs(
            id.trim(),
            request.limit,
            request.offset,
    ...
    async fn album_tracks(
        ...
        if !(1..=100).contains(&request.limit) {
            return Err(TuneWeaveError::invalid_request(
                "QQ album track page size must be between 1 and 100",
            )
            .with_platform(Platform::Qq));
        }
        let (api_request, identifier) = qq_album_songs_request(id, request.limit, request.offset)?;
        let response = self
            .client
            .request_android(&[api_request])
            .await?
    ...
    async fn set_album_subscription(
        ...
        let mut results = self
            .set_album_subscriptions(&[id.to_owned()], subscribed, account)
            .await?;
        results
            .pop()
            .ok_or_else(|| qq_data_error("QQ album subscription returned no result"))
    }
    ...
    async fn set_album_subscriptions(
        ...
        let response = self
            .client
            .request_android_business_with_credential(request, &[], Some(&credential))
            .await?;
        map_qq_album_subscriptions(&album_ids, subscribed, response)
    }
    ...
    async fn ai_lyric_dictionary(
        ...
        let response = self
            .client
            .request_android(&[ai_lyric_dictionary_request(song_id)])
            .await?
            .into_iter()
            .next()
            .ok_or_else(|| qq_data_error("QQ AI lyric dictionary request returned no response"))?;
        map_ai_lyric_dictionary(identifier, song_id, response)
    }
    ...
    async fn account_dislikes(
        ...
        if !(1..=1_000_000).contains(&request.page) {
            return Err(TuneWeaveError::invalid_request(
                "QQ dislike page must be between 1 and 1000000",
            )
            .with_platform(Platform::Qq));
        }
        let account = request.account.as_deref().unwrap_or("default");
        ...
        let response = self
            .client
            .request_android_signed_with_credential(
                &[qq_dislike_list_request(request)?],
                &credential,
            )
            .await?
            .into_iter()
...
fn ai_lyric_dictionary_request(song_id: u64) -> QqApiRequest {
...
fn qq_album_songs_request(
    value: &str,
    limit: u32,
    offset: u32,
) -> Result<(QqApiRequest, QqAlbumIdentifier)> {
    let identifier = parse_qq_album_identifier(value)?;
    let mut param = json!({ "begin": offset, "num": limit });
    match &identifier {
        QqAlbumIdentifier::Numeric(id) => param["albumId"] = json!(id),
        QqAlbumIdentifier::Mid(mid) => param["albumMid"] = json!(mid),
    }
    Ok((
...
fn qq_singer_mvs_request(mid: &str, limit: u32, offset: u32) -> Result<QqApiRequest> {
    let mid = mid.trim();
    validate_qq_media_id(mid, "singer MID")?;
    Ok(QqApiRequest::new(
        SINGER_MV_MODULE,
...
fn map_ai_lyric_dictionary(
    requested: QqTrackIdentifier,
    song_id: u64,
    response: QqApiResponse,
) -> Result<AiLyricDictionary> {
    let parsed = serde_json::from_value::<QqAiDictResponse>(response.data.clone())
        .map_err(|_| qq_data_error("QQ AI lyric dictionary response is malformed"))?;
    let preserved_data = json!(&parsed);
    let entries = parsed
        .entries
        .into_iter()
        .map(|item| {
            ...
            AiLyricDictionaryEntry {
                ...
                translated_lyric_text: item.trans_lyric_text,
                lyric_timestamp: item.lyric_timestamp,
                extensions: Extensions::from([("response".to_owned(), preserved_item)]),
            }
        })
        .collect();
    ...
    Ok(AiLyricDictionary {
        track_ref: qq_ref(&track_id, "AI lyric dictionary")?,
        entries,
        extensions: Extensions::from([
            ("numeric_id".to_owned(), json!(song_id)),
            ("data".to_owned(), preserved_data),
...
fn map_qq_singer_mvs(
    singer_mid: &str,
    limit: u32,
    offset: u32,
    requested_kind: VideoKind,
    response: QqApiResponse,
) -> Result<Page<Video>> {
    validate_qq_media_id(singer_mid, "singer MID")?;
    let data = serde_json::from_value::<QqSingerMvResponse>(response.data)
        .map_err(|error| qq_data_error(format!("QQ singer MV response is malformed: {error}")))?;
    let upstream_returned = u32::try_from(data.videos.len())
        .map_err(|_| qq_data_error("QQ singer MV page is too large"))?;
    let upstream_end = offset.checked_add(upstream_returned).ok_or_else(|| {
        qq_data_error("QQ singer MV pagination exceeded the supported offset range")
    })?;
    if u64::from(upstream_end) > data.total
        || (upstream_returned == 0 && u64::from(offset) < data.total)
    {
        return Err(qq_data_error(
            "QQ singer MV response has inconsistent pagination",
        ));
    }
    let returned = upstream_returned.min(limit);
    let next_offset = offset.checked_add(returned).ok_or_else(|| {
    ...
    let response_data = serde_json::to_value(&data)
        .map_err(|_| qq_data_error("failed to preserve typed QQ singer MV response"))?;
    let returned = usize::try_from(returned)
        .map_err(|_| qq_data_error("QQ singer MV page exceeds this platform's capacity"))?;
    let mut items = Vec::with_capacity(returned);
    for video in data.videos.into_iter().take(returned) {
        items.push(map_qq_singer_mv(singer_mid, video)?);
    }
    let has_more = u64::from(next_offset) < data.total;
    Ok(Page {
        items,
        pagination: PageMeta {
            ...
            extensions: Extensions::from([
                ...
                ("requested_kind".to_owned(), json!(requested_kind)),
                ("order".to_owned(), json!("hot")),
                ("upstream_returned".to_owned(), json!(upstream_returned)),
                ("limit_applied".to_owned(), json!(true)),
                ("data".to_owned(), response_data),
...
fn map_qq_singer_mv(singer_mid: &str, video: QqSingerMv) -> Result<Video> {
    let vid = video.vid.trim();
    if !vid.is_empty() {
        validate_qq_media_id(vid, "MV VID")
            .map_err(|_| qq_data_error("QQ singer MV response returned an invalid VID"))?;
    }
    let id = if !vid.is_empty() {
        ...
        return Err(qq_data_error(
            "QQ singer MV response is missing both MV ID and VID",
        ));
    };
    let title = [video.title.trim(), video.name.trim()]
        .into_iter()
        .find(|title| !title.is_empty())
        .ok_or_else(|| qq_data_error("QQ singer MV response contains a video without a title"))?
        .to_owned();
    let duration_ms = (video.duration > 0)
        .then(|| {
            video
                .duration
                .checked_mul(1_000)
                .ok_or_else(|| qq_data_error("QQ singer MV duration is out of range"))
        })
        .transpose()?;
    let video_data = serde_json::to_value(&video)
        .map_err(|_| qq_data_error("failed to preserve typed QQ singer MV entry"))?;
    let mut extensions = Extensions::from([
        ("singer_mid".to_owned(), json!(singer_mid)),
        ("vid".to_owned(), json!(video.vid)),
        ("mv_type".to_owned(), json!(video.video_type)),
        ("pic_format".to_owned(), json!(video.picformat)),
        ("icon_type".to_owned(), json!(video.icon_type)),
        ("published_at_unix_seconds".to_owned(), json!(video.pubdate)),
        ("singer_mv_entry".to_owned(), video_data),
    ]);
    if video.id > 0 {
...
fn map_qq_album_songs(
    requested: &QqAlbumIdentifier,
    limit: u32,
    offset: u32,
    response: QqApiResponse,
) -> Result<Page<Track>> {
    let data = serde_json::from_value::<QqAlbumSongResponse>(response.data)
        .map_err(|_| qq_data_error("QQ album song response is malformed"))?;
    let album_mid = data.album_mid.trim();
    validate_qq_media_id(album_mid, "album MID")
        .map_err(|_| qq_data_error("QQ album song response contains an invalid album MID"))?;
    if let QqAlbumIdentifier::Mid(requested_mid) = requested
        && album_mid != requested_mid
    {
        return Err(qq_data_error(
            "QQ album song response returned a different album MID",
        ));
    }
    let returned = u32::try_from(data.songs.len())
        .map_err(|_| qq_data_error("QQ album song page is too large"))?;
    if returned > limit {
        return Err(qq_data_error(
            "QQ album song response exceeded the requested page size",
        ));
    }
    let next_offset = offset.checked_add(returned).ok_or_else(|| {
        qq_data_error("QQ album song pagination exceeded the supported offset range")
    })?;
    if u64::from(next_offset) > data.total || (returned == 0 && u64::from(offset) < data.total) {
        return Err(qq_data_error(
            "QQ album song response has inconsistent pagination",
        ));
    }
    let response_data = serde_json::to_value(&data)
        .map_err(|_| qq_data_error("failed to preserve typed QQ album song response"))?;
    let mut items = Vec::with_capacity(data.songs.len());
    for entry in data.songs {
        let entry_data = serde_json::to_value(&entry)
            .map_err(|_| qq_data_error("failed to preserve typed QQ album song entry"))?;
        let mut track = map_track(entry.song)?;
        if let Some(resource_ref) = track
            .album
            .as_ref()
            .and_then(|album| album.resource_ref.as_ref())
            && (resource_ref.platform() != Platform::Qq || resource_ref.id() != album_mid)
        {
            return Err(qq_data_error(
                "QQ album song response contains a track from a different album",
            ));
        }
        track
            .extensions
            .insert("album_song_entry".to_owned(), entry_data);
        items.push(track);
    }
    let has_more = u64::from(next_offset) < data.total;
    Ok(Page {
        ...
        pagination: PageMeta {
            ...
            has_more,
            extensions: Extensions::from([
                ("album_mid".to_owned(), json!(album_mid)),
                (
                    "requested_identifier_kind".to_owned(),
                    json!(match requested {
                        QqAlbumIdentifier::Numeric(_) => "numeric_id",
...
const fn qq_dislike_kind_contract(kind: AccountDislikeKind) -> (i64, &'static str, i64) {
    match kind {
        AccountDislikeKind::Track => (3, "SongLastid", 1),
        AccountDislikeKind::Artist => (2, "SingersLastid", 2),
        AccountDislikeKind::Style => (4, "StyleLastid", 3),
    }
}

fn qq_dislike_list_request(request: &AccountDislikeListRequest) -> Result<QqApiRequest> {
    if !(1..=1_000_000).contains(&request.page) {
        return Err(TuneWeaveError::invalid_request(
            "QQ dislike page must be between 1 and 1000000",
        )
    ...
    let (command, cursor_field, _) = qq_dislike_kind_contract(request.kind);
    let mut param = json!({
        "Cmd": command,
        "Page": request.page
    });
    if let Some(cursor) = request.cursor.filter(|cursor| *cursor > 0) {
        param
            .as_object_mut()
            .expect("QQ dislike parameters are always an object")
            .insert(cursor_field.to_owned(), json!(cursor));
    }
    Ok(QqApiRequest::new(
        DISLIKE_MODULE,
        DISLIKE_LIST_METHOD,
...
fn map_qq_dislike_list(
    request: &AccountDislikeListRequest,
    response: QqApiResponse,
) -> Result<AccountDislikeList> {
    let data = serde_json::from_value::<QqDislikeListResponse>(response.data).map_err(|error| {
        qq_data_error(format!("QQ dislike list response is malformed: {error}"))
    })?;
    if data.retcode != 0 {
        return Err(TuneWeaveError::new(
            ErrorCode::UpstreamError,
            format!("QQ dislike list failed with code {}", data.retcode),
        )
        .with_platform(Platform::Qq)
        .with_details(json!({
    ...
    let (selected, unexpected_counts) = match request.kind {
        AccountDislikeKind::Track => (&data.tracks, [data.artists.len(), data.styles.len()]),
        AccountDislikeKind::Artist => (&data.artists, [data.tracks.len(), data.styles.len()]),
        AccountDislikeKind::Style => (&data.styles, [data.tracks.len(), data.artists.len()]),
    };
    if unexpected_counts.into_iter().any(|count| count > 0) {
        return Err(qq_data_error(
            "QQ dislike list returned entries outside the requested kind",
        ));
    }
    let items = selected
        .iter()
        .cloned()
        .map(|item| map_qq_dislike_item(request.kind, item))
        .collect::<Result<Vec<_>>>()?;
    let next_cursor = selected
        .last()
        .map(|item| qq_response_positive_u64(&item.id, "dislike continuation ID"))
        .transpose()?;
    let next_page = if items.is_empty() {
        None
    } else {
        Some(request.page.checked_add(1).ok_or_else(|| {
            qq_data_error("QQ dislike pagination exceeded the supported page range")
        })?)
    };
    let token = (!data.token.trim().is_empty()).then(|| data.token.trim().to_owned());
    let typed_data = serde_json::to_value(&data)
        .map_err(|_| qq_data_error("failed to preserve typed QQ dislike list response"))?;
...
fn map_qq_dislike_item(
    ...
    let name = item.name.trim();
    if name.is_empty() {
        return Err(qq_data_error("QQ dislike item is missing its name"));
    }
    let image_url = first_qq_display_url(&[(item.image_url.as_str(), "dislike item image")])?;
    let added_at = qq_platform_timestamp_rfc3339(item.timestamp);
    if item.timestamp > 0 && added_at.is_none() {
        return Err(qq_data_error(
            "QQ dislike item returned an unsupported timestamp",
        ));
    }
    let raw = serde_json::to_value(&item)
        .map_err(|_| qq_data_error("failed to preserve typed QQ dislike item"))?;
    Ok(AccountDislikeEntry {
        platform: Platform::Qq,
        kind,
        id: id.to_string(),
        name: name.to_owned(),
...
fn qq_response_positive_u64(value: &str, context: &str) -> Result<u64> {
    ...
    if value.is_empty() || !value.bytes().all(|byte| byte.is_ascii_digit()) {
        return Err(qq_data_error(format!(
            "QQ {context} is not a positive unsigned integer"
        )));
    }
    value
        .parse::<u64>()
        .ok()
        .filter(|value| *value > 0)
        .ok_or_else(|| qq_data_error(format!("QQ {context} is not a positive unsigned integer")))
}
...
fn qq_album_subscription_request(
    ...
    let album_ids = ids
        .iter()
        .map(|id| parse_qq_playlist_number(id, "album ID"))
        .collect::<Result<Vec<_>>>()?;
    Ok((
        QqApiRequest::new(
...
fn map_qq_album_subscriptions(
    ...
    if response.code != 0 {
        return Err(TuneWeaveError::new(
            ErrorCode::UpstreamError,
            "QQ album subscription was rejected",
        )
        .with_platform(Platform::Qq)
        .with_details(json!({
    ...
    let parsed = serde_json::from_value::<QqAlbumSubscriptionResponse>(response.data.clone())
        .map_err(|_| qq_data_error("QQ album subscription response is malformed"))?;
    if parsed.result != 0 {
        return Err(TuneWeaveError::new(
            ErrorCode::UpstreamError,
            "QQ album subscription was rejected",
        ...
        .with_details(json!({
            ...
            "subscribed": subscribed,
            "album_ids": album_ids,
            "failed_album_ids": parsed.failed_album_ids
        })));
    }
    if parsed
        .failed_album_ids
        .iter()
        .any(|failed_id| *failed_id == 0 || !album_ids.contains(failed_id))
    {
        return Err(qq_data_error(
            "QQ album subscription returned an unknown failed album ID",
        ));
    }
    if !parsed.failed_album_ids.is_empty() {
        return Err(TuneWeaveError::new(
            ErrorCode::Conflict,
            "QQ album subscription did not change every requested album",
        )
        .with_platform(Platform::Qq)
        .with_details(json!({
            "platform_code": response.code,
    ...
    let data = serde_json::to_value(&parsed)
        .map_err(|_| qq_data_error("failed to preserve typed QQ album subscription response"))?;
    album_ids
        .iter()
        .enumerate()
        .map(|(index, album_id)| {
            Ok(SubscriptionResult {
                resource_ref: qq_ref(&album_id.to_string(), "album subscription")?,
                subscribed,
                extensions: Extensions::from([
                    (
                        "action".to_owned(),
                    ...
                    ("numeric_id".to_owned(), json!(album_id)),
                    ("batch_index".to_owned(), json!(index)),
                    ("requested_album_ids".to_owned(), json!(album_ids)),
                    (
                        "failed_album_ids".to_owned(),
                        json!(parsed.failed_album_ids),
                    ),
...
mod tests {
    ...
    fn sample_dislike_item(id: u64, id_type: i64, name: &str) -> Value {
        json!({
            "ID": id.to_string(),
            "Name": name,
            "Img": "//y.qq.com/dislike/image.jpg",
            "IdType": id_type,
            "Time": 1_704_067_200_u64,
            "futureDislikeField": {"kept": true}
        })
    }
    ...
    fn sample_singer_mv(id: i64, vid: &str, title: &str) -> Value {
        json!({
            "mvid": id,
            "vid": vid,
            "type": "0",
    ...
    async fn singing_annotations_availability_rejects_invalid_identity_and_account_before_network()
    {
        ...
        let account_error = provider
            .ai_lyric_dictionary("7137686", Some("missing-account"))
            .await
            .expect_err("AI dictionary detail missing named account");
        assert_eq!(account_error.code, ErrorCode::AuthenticationRequired);

        let identity_error = provider
            .ai_lyric_dictionary("0", None)
            .await
            .expect_err("AI dictionary detail zero numeric track ID");
        assert_eq!(identity_error.code, ErrorCode::InvalidRequest);
    }
    ...
    #[test]
    fn ai_lyric_dictionary_request_and_mapping_preserve_complete_ordered_entries() {
        let request = ai_lyric_dictionary_request(7_137_686);
        assert_eq!(request.module, LYRIC_MODULE);
        assert_eq!(request.method, "GetAIDictInfo");
        assert_eq!(request.param, json!({"songID": 7_137_686_u64}));

        let mapped = map_ai_lyric_dictionary(
            QqTrackIdentifier::Mid("002uFQ2G2zxj1n".to_owned()),
            7_137_686,
            response(json!({
                "dictList": [
                    {
                        ...
                        "explain": "第一条详细解释",
                        "lyric_text": "第一行原文",
                        "trans_lyric_text": "第一行翻译",
                        "lyric_timestamp": "0000001000",
                        "futureItemField": "first"
                    },
                    {
                        "phrase": "第二个短语",
                        "explain": "第二条详细解释",
                        "lyric_text": "第二行原文",
                        "trans_lyric_text": "第二行翻译",
                        "lyric_timestamp": "0000002000"
                    }
                ],
                "futureTopField": true
            })),
        )
        .expect("map AI lyric dictionary");
        assert_eq!(mapped.track_ref.to_string(), "qq:002uFQ2G2zxj1n");
        assert_eq!(mapped.entries.len(), 2);
        assert_eq!(mapped.entries[0].phrase, "第一个短语");
        assert_eq!(mapped.entries[0].explanation, "第一条详细解释");
        assert_eq!(mapped.entries[0].lyric_text, "第一行原文");
        assert_eq!(mapped.entries[0].translated_lyric_text, "第一行翻译");
        assert_eq!(mapped.entries[0].lyric_timestamp, "0000001000");
        assert_eq!(
            mapped.entries[0].extensions["response"]["futureItemField"],
            "first"
        );
        assert_eq!(mapped.entries[1].phrase, "第二个短语");
        assert_eq!(mapped.extensions["numeric_id"], 7_137_686);
        assert_eq!(mapped.extensions["data"]["futureTopField"], true);
        assert_eq!(mapped.extensions["response"]["code"], 0);

        let empty = map_ai_lyric_dictionary(
            QqTrackIdentifier::Numeric(7_137_686),
            7_137_686,
            response(json!({})),
        )
        .expect("missing dictionary list follows upstream empty default");
        assert_eq!(empty.track_ref.to_string(), "qq:7137686");
        assert!(empty.entries.is_empty());

        let defaults = map_ai_lyric_dictionary(
            QqTrackIdentifier::Numeric(7_137_686),
            7_137_686,
            response(json!({"dictList": [{}]})),
        )
        .expect("missing item strings follow upstream empty defaults");
        assert_eq!(defaults.entries.len(), 1);
        assert!(defaults.entries[0].phrase.is_empty());
        assert!(defaults.entries[0].explanation.is_empty());
        assert!(defaults.entries[0].lyric_timestamp.is_empty());
    }

    #[test]
    fn ai_lyric_dictionary_mapping_rejects_malformed_container_and_fields() {
        for malformed in [
            json!({"dictList": {}}),
            json!({"dictList": [{"phrase": 1}]}),
            json!({"dictList": [{"explain": []}]}),
            json!({"dictList": [{"lyric_text": {}}]}),
            json!({"dictList": [{"trans_lyric_text": false}]}),
            json!({"dictList": [{"lyric_timestamp": 1000}]}),
        ] {
            let error = map_ai_lyric_dictionary(
                QqTrackIdentifier::Numeric(7_137_686),
                7_137_686,
                response(malformed),
            )
            .expect_err("malformed AI lyric dictionary response");
            assert_eq!(error.code, ErrorCode::UpstreamError);
        }
    }
    ...
    #[test]
    fn singer_mv_request_preserves_hot_order_and_exact_offset() {
        let request = qq_singer_mvs_request(" 0025NhlN2yWrP4 ", 30, 7).expect("singer MV request");
        assert_eq!(request.module, SINGER_MV_MODULE);
        assert_eq!(request.method, SINGER_MV_METHOD);
        assert_eq!(
            request.param,
            json!({
                "singermid": "0025NhlN2yWrP4",
    ...
    #[test]
    fn singer_mv_mapping_preserves_typed_items_overfetch_and_real_pagination() {
        let page = map_qq_singer_mvs(
            "0025NhlN2yWrP4",
            2,
            0,
            VideoKind::Mv,
            response(json!({
                "total": "5",
                "list": [
                    sample_singer_mv(101, "a001mvvid01", "晴天 MV"),
                    sample_singer_mv(102, "a001mvvid02", "东风破 MV"),
                    sample_singer_mv(103, "a001mvvid03", "夜曲 MV")
                ],
                "futurePageField": "kept"
            })),
        )
        .expect("map singer MV page");
        assert_eq!(page.items.len(), 2);
        assert_eq!(page.items[0].resource_ref.to_string(), "qq:a001mvvid01");
        assert_eq!(page.items[0].title, "晴天 MV");
        assert!(page.items[0].creators.is_empty());
        ...
        assert_eq!(page.items[0].extensions["singer_mid"], "0025NhlN2yWrP4");
        assert_eq!(
            page.items[0].extensions["singer_mv_entry"]["futureMvField"],
            true
        );
        assert_eq!(page.pagination.total, Some(5));
        assert_eq!(page.pagination.next_offset, Some(2));
        assert!(page.pagination.has_more);
        assert_eq!(page.pagination.extensions["kind"], "mv");
        assert_eq!(page.pagination.extensions["requested_kind"], "mv");
        assert_eq!(page.pagination.extensions["order"], "hot");
        assert_eq!(page.pagination.extensions["upstream_returned"], 3);
        assert_eq!(page.pagination.extensions["limit_applied"], true);
        assert_eq!(
        ...
        .expect("null MV list follows the upstream empty-list contract");
        assert!(empty.items.is_empty());
        assert_eq!(empty.pagination.total, Some(0));
        assert_eq!(empty.pagination.extensions["requested_kind"], "all");
        assert!(!empty.pagination.has_more);
    }

    #[test]
    fn singer_mv_mapping_rejects_malformed_items_and_pagination() {
        for (offset, data) in [
            (0, json!({"total": "many", "list": []})),
            (0, json!({"total": 1, "list": {}})),
            (0, json!({"total": 1, "list": [{}]})),
            (
                0,
                json!({"total": 1, "list": [sample_singer_mv(101, "unsafe/vid", "晴天 MV")]}),
            ),
            (
                0,
                json!({"total": 1, "list": [sample_singer_mv(0, "", "晴天 MV")]}),
            ),
            (
                0,
                json!({"total": 1, "list": [sample_singer_mv(101, "a001mvvid01", " ")]}),
            ),
            (0, json!({"total": 1, "list": []})),
            (
                1,
                json!({"total": 1, "list": [sample_singer_mv(101, "a001mvvid01", "晴天 MV")]}),
            ),
        ] {
            let error =
    ...
    fn album_song_request_preserves_numeric_mid_and_exact_offset_pagination() {
        ...
        assert_eq!(identifier, QqAlbumIdentifier::Numeric(100));
        assert_eq!(numeric.module, ALBUM_SONG_MODULE);
        assert_eq!(numeric.method, ALBUM_SONG_METHOD);
        assert_eq!(
            numeric.param,
            json!({"albumId": 100, "begin": 7, "num": 30})
        );

        let (mid, identifier) =
            qq_album_songs_request("000MkMni19ClKG", 5, 10).expect("album MID songs");
        assert_eq!(
            identifier,
            QqAlbumIdentifier::Mid("000MkMni19ClKG".to_owned())
        );
        assert_eq!(
            mid.param,
            json!({"albumMid": "000MkMni19ClKG", "begin": 10, "num": 5})
        );
    }

    #[test]
    fn album_song_mapping_preserves_wrappers_tracks_and_real_pagination() {
        let page = map_qq_album_songs(
            &QqAlbumIdentifier::Mid("000MkMni19ClKG".to_owned()),
            2,
            0,
            response(json!({
        ...
        .expect("map QQ album song page");
        assert_eq!(page.items.len(), 2);
        assert_eq!(page.items[0].resource_ref.to_string(), "qq:0039MnYb0qxYhV");
        assert_eq!(page.items[1].resource_ref.to_string(), "qq:003v4UL61IYlTY");
        assert_eq!(page.items[0].extensions["album_song_entry"]["index"], 1);
        assert_eq!(
            page.items[0].extensions["album_song_entry"]["futureEntryField"],
            true
        );
        assert_eq!(page.pagination.limit, 2);
        assert_eq!(page.pagination.offset, 0);
        ...
        assert_eq!(page.pagination.next_offset, Some(2));
        assert!(page.pagination.has_more);
        assert_eq!(page.pagination.extensions["album_mid"], "000MkMni19ClKG");
        assert_eq!(
            page.pagination.extensions["data"]["futurePageField"],
            "kept"
        );
    ...
    #[test]
    fn album_song_mapping_rejects_malformed_wrappers_identity_and_pagination() {
        let identifier = QqAlbumIdentifier::Mid("000MkMni19ClKG".to_owned());
        for (limit, offset, data) in [
            (10, 0, json!({"totalNum": 0, "songList": []})),
            (
                10,
                0,
                json!({"albumMid": "differentMid", "totalNum": 0, "songList": []}),
            ),
            (
                10,
                0,
                json!({"albumMid": "000MkMni19ClKG", "totalNum": "many", "songList": []}),
            ),
            (
                10,
                0,
                json!({"albumMid": "000MkMni19ClKG", "totalNum": 1, "songList": {}}),
            ),
            (
                10,
                0,
            ...
            (
                1,
                0,
                json!({"albumMid": "000MkMni19ClKG", "totalNum": 2, "songList": [
                    {"songInfo": sample_track(97_773, "0039MnYb0qxYhV", "晴天")},
                    {"songInfo": sample_track(48_283, "003v4UL61IYlTY", "东风破")}
                ]}),
            ),
            ...
            let error = map_qq_album_songs(&identifier, limit, offset, response(data))
                .expect_err("malformed QQ album song response");
            assert_eq!(error.code, ErrorCode::UpstreamError);
        }
    }
    ...
    #[test]
    fn dislike_list_requests_preserve_each_command_cursor_and_page_branch() {
        for (kind, command, cursor_field) in [
            (AccountDislikeKind::Track, 3, "SongLastid"),
            (AccountDislikeKind::Artist, 2, "SingersLastid"),
            (AccountDislikeKind::Style, 4, "StyleLastid"),
        ] {
            let request = qq_dislike_list_request(&AccountDislikeListRequest {
                kind,
                page: 3,
                cursor: Some(123),
                account: Some("personal".to_owned()),
            })
            .expect("dislike list request");
            assert_eq!(request.module, DISLIKE_MODULE);
            assert_eq!(request.method, DISLIKE_LIST_METHOD);
            assert_eq!(request.param["Cmd"], command);
            assert_eq!(request.param["Page"], 3);
            assert_eq!(request.param[cursor_field], 123);
            for other in ["SongLastid", "SingersLastid", "StyleLastid"] {
                if other != cursor_field {
                    assert!(request.param.get(other).is_none());
                }
            }
        }

        let first = qq_dislike_list_request(&AccountDislikeListRequest::default())
            .expect("first dislike page");
        assert_eq!(first.param, json!({"Cmd": 3, "Page": 1}));
        let zero_cursor = qq_dislike_list_request(&AccountDislikeListRequest {
            cursor: Some(0),
            ..AccountDislikeListRequest::default()
        })
        .expect("reference zero cursor is omitted");
        assert!(zero_cursor.param.get("SongLastid").is_none());
        for page in [0, 1_000_001] {
            assert!(
                qq_dislike_list_request(&AccountDislikeListRequest {
                    page,
                    ..AccountDislikeListRequest::default()
                })
                .is_err()
    ...
    #[test]
    fn dislike_list_mapping_preserves_typed_items_and_inferred_continuation() {
        let request = AccountDislikeListRequest {
            kind: AccountDislikeKind::Track,
            page: 2,
            cursor: Some(300),
            account: Some("personal".to_owned()),
        };
        let list = map_qq_dislike_list(
            &request,
            response(json!({
                "Retcode": 0,
                "Msg": "ok",
                ...
                "futureListField": {"kept": true}
            })),
        )
        .expect("map QQ dislike list");

        assert_eq!(list.platform, Platform::Qq);
        assert_eq!(list.kind, AccountDislikeKind::Track);
        assert_eq!(list.items.len(), 2);
        assert_eq!(list.items[0].id, "398282803");
        assert_eq!(list.items[0].name, "不喜欢歌曲");
        assert_eq!(
            list.items[0].image_url.as_deref(),
            Some("https://y.qq.com/dislike/image.jpg")
        );
        assert_eq!(
        ...
        assert_eq!(list.next_page, Some(3));
        assert_eq!(list.next_cursor, Some(399_000_001));
        assert_eq!(list.token.as_deref(), Some("opaque-page-token"));
        assert_eq!(list.extensions["requested_cursor"], 300);
        assert_eq!(list.extensions["continuation_inferred"], true);
        assert_eq!(list.extensions["data"]["futureListField"]["kept"], true);
        assert_eq!(list.extensions["response"]["code"], 0);

        let end = map_qq_dislike_list(
            &AccountDislikeListRequest {
        ...
        .expect("empty dislike page");
        assert!(end.items.is_empty());
        assert_eq!(end.next_page, None);
    ...
    #[test]
    fn dislike_list_mapping_rejects_errors_wrong_kinds_and_malformed_items() {
        let request = AccountDislikeListRequest::default();
        let valid = || {
            json!({
                "Retcode": 0,
                "Msg": "",
                "Singers": [],
                "Songs": [sample_dislike_item(398_282_803, 1, "歌曲")],
                "Styles": [],
                "Page": 1,
                "Token": ""
            })
        ...
        let mut fixtures = vec![
            json!({}),
            json!({
                "Retcode": 1000, "Msg": "login required",
                "Singers": [], "Songs": [], "Styles": [], "Page": 1, "Token": ""
            }),
            json!({
                "Retcode": 0, "Msg": "",
                "Singers": [sample_dislike_item(4558, 2, "歌手")],
                "Songs": [sample_dislike_item(398_282_803, 1, "歌曲")],
                "Styles": [], "Page": 1, "Token": ""
            }),
        ];
        for (field, value) in [
            ...
            ("ID", json!("not-numeric")),
            ("Name", json!("")),
            ("Img", json!("file:///secret")),
            ("IdType", json!(2)),
            ("Time", json!(-1)),
        ] {
            let mut fixture = valid();
            fixture["Songs"][0][field] = value;
            fixtures.push(fixture);
        }
        for (field, value) in [
            ("Page", json!(2)),
            ("Songs", json!({})),
            ("Retcode", json!("invalid")),
        ] {
            let mut fixture = valid();
            fixture[field] = value;
            fixtures.push(fixture);
        ...
        for fixture in fixtures {
            let error = map_qq_dislike_list(&request, response(fixture))
                .expect_err("invalid QQ dislike response");
            assert_eq!(error.code, ErrorCode::UpstreamError);
        }
    }
    ...
    async fn dislike_list_requires_the_exact_account_before_signed_network_access() {
        ...
        let missing = provider
            ...
            .await
            .expect_err("missing account rejected before signed request");
        assert_eq!(missing.code, ErrorCode::AuthenticationRequired);
        assert_eq!(missing.details["account"], "missing");
    ...
    #[test]
    fn album_favorite_mapping_preserves_batch_results_and_rejects_failed_ids() {
        let ids = vec![100, 7, 100];
        let results = map_qq_album_subscriptions(
            &ids,
            true,
            QqBusinessResponse {
                code: 0,
                data: json!({
                    "result": 0,
                    "v_failedAlbumId": [],
                    "futureWriteField": {"kept": true}
                }),
                raw: json!({"code": 0, "data": {"result": 0}}),
            },
        )
        .expect("QQ album favorite result");
        assert_eq!(results.len(), 3);
        assert_eq!(results[0].resource_ref.to_string(), "qq:100");
        assert_eq!(results[1].resource_ref.to_string(), "qq:7");
        assert_eq!(results[2].resource_ref.to_string(), "qq:100");
        assert!(results.iter().all(|result| result.subscribed));
        assert_eq!(results[1].extensions["batch_index"], 1);
        assert_eq!(
            results[0].extensions["requested_album_ids"],
            json!([100, 7, 100])
        );
        assert_eq!(
        ...
        let conflict = map_qq_album_subscriptions(
            ...
            QqBusinessResponse {
                code: 0,
                data: json!({"result": 0, "v_failedAlbumId": [7]}),
                raw: json!({"code": 0}),
            },
        )
        .expect_err("partial QQ album favorite result");
        assert_eq!(conflict.code, ErrorCode::Conflict);
        assert_eq!(conflict.details["failed_album_ids"], json!([7]));

        let unknown = map_qq_album_subscriptions(
            &ids,
            true,
            QqBusinessResponse {
                code: 0,
                data: json!({"result": 0, "v_failedAlbumId": [999]}),
                raw: json!({"code": 0}),
            },
        )
        .expect_err("unknown QQ failed album ID");
        assert_eq!(unknown.code, ErrorCode::UpstreamError);
        ...
        for response in [
            ...
            QqBusinessResponse {
                code: 0,
                data: json!({"result": 1000, "v_failedAlbumId": []}),
                raw: json!({"code": 0}),
            },
            QqBusinessResponse {
                code: 0,
            ...
            let error = map_qq_album_subscriptions(&ids, true, response)
                .expect_err("rejected or malformed QQ album favorite result");
            assert_eq!(error.code, ErrorCode::UpstreamError);
        }
    }
    ...
    async fn album_detail_validates_identifiers_and_accounts_before_network_access() {
        ...
        let page_size = provider
            ...
            .await
            .expect_err("invalid album track page size");
        assert_eq!(page_size.code, ErrorCode::InvalidRequest);
        let page_account = provider
            .album_tracks(
                "100",
                &tuneweave_core::PageRequest {
                    limit: 10,
    ...
    async fn singer_homepage_validates_mids_and_accounts_before_network_access() {
        ...
        let mut videos = ArtistVideoListRequest::new(0, 0);
        let error = provider
            .artist_videos("0025NhlN2yWrP4", &videos)
            .await
            .expect_err("invalid singer MV page size");
        assert_eq!(error.code, ErrorCode::InvalidRequest);
        videos.limit = 10;
        videos.cursor = Some("cursor".to_owned());
        let error = provider
            .artist_videos("0025NhlN2yWrP4", &videos)
            .await
            .expect_err("unsupported singer MV cursor");
        assert_eq!(error.code, ErrorCode::InvalidRequest);
        videos.cursor = None;
        videos.order = Some("time".to_owned());
        let error = provider
            .artist_videos("0025NhlN2yWrP4", &videos)
            .await
            .expect_err("unsupported singer MV order");
        assert_eq!(error.code, ErrorCode::InvalidRequest);
        assert_eq!(error.details["allowed_order"], "hot");
        videos.order = Some("HOT".to_owned());
        videos.account = Some("missing-account".to_owned());
        let error = provider
            .artist_videos("0025NhlN2yWrP4", &videos)
            .await
            .expect_err("missing singer MV account alias");
        assert_eq!(error.code, ErrorCode::AuthenticationRequired);
    }
    ...
    async fn album_subscription_validates_actions_batch_and_exact_account_before_network_access() {
        ...
        for ids in [Vec::new(), vec!["not-numeric".to_owned()]] {
            let invalid = provider
                .set_album_subscriptions(&ids, true, Some("missing-account"))
                .await
    ...
    #[tokio::test]
    #[ignore = "requires live QQ Music services"]
    async fn live_singer_mv_pages_preserve_exact_offsets_and_continuation() {
        let provider = QqProvider::new(QqConfig {
            device_path: std::env::var_os("TUNEWEAVE_QQ_LIVE_DEVICE").map(Into::into),
            ..QqConfig::default()
        })
        .expect("provider");
        let request = |offset| {
            let mut request = ArtistVideoListRequest::new(2, offset);
            request.kind = VideoKind::Mv;
            request
        };
        let first = provider
            .artist_videos("0025NhlN2yWrP4", &request(0))
            .await
            .expect("first singer MV page");
        assert_eq!(first.items.len(), 2);
        assert!(first.pagination.total.is_some_and(|total| total > 2));
        assert_eq!(first.pagination.next_offset, Some(2));
        assert_eq!(first.pagination.extensions["singer_mid"], "0025NhlN2yWrP4");
        ...
        let shifted = provider
            .artist_videos("0025NhlN2yWrP4", &request(1))
            .await
            .expect("non-aligned singer MV page");
        assert_eq!(shifted.items.len(), 2);
        assert_eq!(shifted.items[0].resource_ref, first.items[1].resource_ref);
    ...
    #[tokio::test]
    #[ignore = "requires live QQ Music services"]
    async fn live_album_song_pages_accept_numeric_id_mid_and_continue_without_overlap() {
        let provider = QqProvider::new(QqConfig {
            device_path: std::env::var_os("TUNEWEAVE_QQ_LIVE_DEVICE").map(Into::into),
            ..QqConfig::default()
        })
        ...
        let numeric = provider
            .album_tracks("8220", &request)
            .await
            .expect("numeric album song page");
        assert!(!numeric.items.is_empty());
        assert!(numeric.pagination.total.is_some_and(|total| total > 0));
        let album_mid = numeric.pagination.extensions["album_mid"]
            .as_str()
            .expect("album MID");
        let by_mid = provider
            .album_tracks(album_mid, &request)
            .await
            .expect("album MID song page");
        assert_eq!(by_mid.items, numeric.items);
        assert_eq!(by_mid.pagination.total, numeric.pagination.total);

        if let Some(next_offset) = numeric.pagination.next_offset {
            let next = provider
                .album_tracks(
                    album_mid,
                    &tuneweave_core::PageRequest {
                ...
                .await
                .expect("next album song page");
            assert!(!next.items.is_empty());
            assert!(numeric.items.iter().all(|first| {
                next.items
                    .iter()
                    .all(|second| first.resource_ref != second.resource_ref)
            }));
        }
    }
    ...
    #[tokio::test]
    #[ignore = "requires live QQ Music services"]
    async fn live_ai_lyric_dictionary_accepts_numeric_id_and_mid() {
        let provider = QqProvider::new(QqConfig {
            device_path: std::env::var_os("TUNEWEAVE_QQ_LIVE_DEVICE").map(Into::into),
            ..QqConfig::default()
        })
        .expect("provider");
        let numeric = provider
            .ai_lyric_dictionary("7137686", None)
            .await
            .expect("numeric AI lyric dictionary");
        assert_eq!(numeric.track_ref.to_string(), "qq:7137686");
        assert!(!numeric.entries.is_empty());
        assert!(numeric.entries.iter().all(|entry| {
            !entry.phrase.is_empty()
                && !entry.explanation.is_empty()
                && !entry.lyric_text.is_empty()
                && !entry.lyric_timestamp.is_empty()
        }));
        assert_eq!(numeric.extensions["response"]["code"], 0);
        ...
        let track = provider
            ...
            .expect("resolve AI dictionary track MID");
        let by_mid = provider
            .ai_lyric_dictionary(&track.id, None)
            .await
            .expect("MID AI lyric dictionary");
        assert_eq!(by_mid.track_ref, track.resource_ref);
        assert_eq!(by_mid.extensions["numeric_id"], 7_137_686);
